...
class PlaylistClusterer:
    ...
    def get_sse_score(self, centroids, labels):
        sse = 0
        for i in range(len(self.features)):
            sse += np.linalg.norm(self.features[i]-centroids[labels[i]], ord=2)
        return sse
    ...
    def split_small_clusters_old(self, labels, small_threshold=3):
        data = self.features
    ...
    def cluster_old(self, k=10, split_small_clusters=False):
        self.k = k
        ...
        if split_small_clusters:
            return self.split_small_clusters_old(self.labels)
        else:
            for i in self.u_labels:
